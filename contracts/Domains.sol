...
// Inheriting imported contract gives access to inherited methods
contract Domains is ERC721URIStorage {
    // global owner variable
    ...
    constructor(string memory _tld)
        payable
        ERC721("GangGang Name Service", "GNS")
    {
    ...
    function lengthPrice(string calldata name) public pure returns (uint256) {
    ...
    function valid(string calldata name) public pure returns (bool) {
        return StringUtils.strlen(name) >= 3 && StringUtils.strlen(name) <= 5;
    }
    ...
    function register(string calldata name) public payable {
        ...
        uint256 _price = this.lengthPrice(name);
        ...
        // Create JSON metadata for NFT: combine strings and -> base64
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the GangGang name Service", "image": "data:image/svg+xml;base64,',
